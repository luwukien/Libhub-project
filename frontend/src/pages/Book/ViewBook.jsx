...
const ViewBook = ({ bookInfo, onClose, onEditClick, onDeleteClick, userInfo }) => {
  return (
    <div className="relative">
      ...
      <div>
        <div className="flex-1 flex flex-col gap-2 py-4">
          <h1 className="text-2xl text-slate-950">{bookInfo?.title}</h1>
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="inline-flex items-center gap-2 text-[13px] text-yellow-600 bg-pornhub-200/30 rounded px-2 py-1">
              ...
              {bookInfo?.category?.map((item, index) =>
                bookInfo.category.length === index + 1 ? `${item}` : `${item}, `
              )}
            </div>
          </div>
        </div>
